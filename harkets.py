...
def baristaView():
    ...
    choice = input("Enter 1 to submit new order, 2 to quit, 3 to Main Menu:\n")
    if choice == "1":
        baristaView()
    elif choice == "2":
        quit()
...
def manageEmployees():
    ...
    if choice == '1':
        ...
        # TODO add so you cant fire yourself
        # TODO delete from other tables also
        ...
        # TODO add change of ownership or works schedule
...
def manageInventory():
    ...
    while True:
        ...
        cur.execute("SELECT ingredient_name, quantity_in_stock, unit, purchase_price, purchase_unit FROM inventory WHERE ingredient_name = %s", (toOrder,))
        results = cur.fetchone()
        while (toOrder != "X" and not results):
        ...
        print(f'{results[0]}:')
        print(f'Currently in inventory: {results[1]} {results[2]}')
        print(f'Purchase price: ${results[3]} per {results[4]} {results[2]}')
        print(f'How many packs would you like to purchase?')
        quantity = int(input())
        purchasedQuanity = quantity * int(results[4])
        purchasedPrice = quantity * float(results[3])
        print(f'Purchased {purchasedQuanity} {results[2]} of {results[0]} for ${purchasedPrice}')

        #Update inventory
        newQuantity = purchasedQuanity + float(result[1])

        cur.execute("UPDATE inventory SET quantity_in_stock = %s WHERE ingredient_name = %s", (newQuantity, results[0]))
        conn.commit()
...
def baristaManagerView():
    ...
    print("2. Manage inventory")
    print("3. Accounting report")
    print("4. Prepare drinks") #if user chooses that it should call the barista view
...
while True:
    ...
    #testing manager view without login
    #managerView() 
